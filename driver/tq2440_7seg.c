...
static void timer_1_2_init(void)
{
	...
	tcfg1 &= ~(S3C2410_TCFG1_MUX3_MASK | S3C2410_TCFG1_MUX0_MASK);
	tcfg1 |= ((S3C2410_TCFG1_MUX3_DIV16) | (S3C2410_TCFG1_MUX0_DIV16));
	...
	__raw_writel(tcntb0, S3C2410_TCNTB(3));

	tcntb1 = (pclk/(256 * 16));		//1s
	__raw_writel(tcntb1, S3C2410_TCNTB(0));
	...
	printk(DEVICE_NAME" timer0 and time3 initialized\n");
}
...
static int tq2440_seg_open(struct inode *inode,struct file *filp)
{
	...
	err1 = request_irq(IRQ_TIMER3, timer2_interrupt,IRQF_DISABLED, "TIMER3", NULL);
	if(err1)
	{
	...
	err2 = request_irq(IRQ_TIMER0, timer1_interrupt,IRQF_DISABLED, "TIMER0", NULL);
	if(err2)
	{
	...
	intmask &= ~((1<<13)|(1<<10));	//timer2 and timer1
